...
An alert fired on an inbound email delivered to j.garcia@thetrydaily.thm from onboarding@hrconnex.thm. The subject line was "Action Required: Finalize Your Onboarding Profile" and the email contained a link to https://hrconnex.thm/onboarding/15400654060/j.garcia. The alert description noted I should check firewall and proxy logs to see if any endpoints attempted to access the URL.
...
A high severity alert fired when a user on 10.20.2.17 attempted to access a URL on the organization's threat intelligence blacklist. The firewall blocked the connection before it completed. The URL was a bit.ly shortlink: http://bit.ly/3sHkX3da12340.
...
The bit.ly link resolves to https://plixaroo.info/ (67.199.248.11). I ran it through VirusTotal.
...
| Phishing Link | http://bit.ly/3sHkX3da12340 |
| Redirect Destination | https://plixaroo.info/ |

**Reason for True Positive Classification:**
Hannah Harris attempted to access a shortened URL (http://bit.ly/3sHkX3da12340) that redirected to https://plixaroo.info/. VirusTotal flagged plixaroo.info as malicious (ADMINUSLabs) and suspicious (alphamountain.ai). The domain had no established content category and used iframes and third-party cookies consistent with phishing infrastructure. The connection was blocked by the firewall before any data exchange occurred.
...
- Blacklist http://bit.ly/3sHkX3da12340 at the proxy level
...
**Reason for True Positive Classification:**
Hannah Harris received a phishing email from urgents@amazon.biz at 06:51:55. The sender domain amazon.biz is not affiliated with Amazon and is consistent with a domain impersonation attempt. The email contained a link (http://bit.ly/3sHkX3da12340) that redirected to https://plixaroo.info/, a domain flagged as malicious on VirusTotal. Harris clicked the link, but the connection was blocked by the firewall before any data exchange occurred.
...
**Reason for True Positive Classification:**
c.allen received a phishing email from no-reply@m1crosoftsupport.co. The sender domain uses typosquatting, substituting "1" for "i" to impersonate Microsoft. Unlike previous alerts, the outbound connection to https://m1crosoftsupport.co/login was allowed by the firewall over HTTPS on port 443. c.allen accessed the link at 06:55:22, indicating potential credential compromise on a fake Microsoft login page.
